...
impl Sha3Type {
    pub fn get_h(&self) -> [u64; 8] {
        // TODO
        match self {
            Self::Sha384 => [0xcbbb9d5dc1059ed8, 0x629a292a367cd507, 0x9159015a3070dd17, 0x152fecd8f70e5939, 0x67332667ffc00b31, 0x8eb44a8768581511, 0xdb0c2e0d64f98fa7, 0x47b5481dbefa4fa4],
            Self::Sha512 => [0x6a09e667f3bcc908,  0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1, 0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179],
        }
    }
    // TODO
    pub fn get_k (&self) -> Vec<u64> {
        vec![0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc,
...
impl Sha3 {
    pub fn sha384() -> Self {
        Self{msg: Vec::new(), sha_type: Sha3Type::Sha384}
    }

    pub fn sha512() -> Self {
        Self{msg: Vec::new(), sha_type: Sha3Type::Sha512}
    }
    ...
    fn padding(&self) -> Vec<u64> {
        let mut bytes = self.msg.clone();
        let n = (888 - (bytes.len() * 8) % 888) / 8;
        let l = (bytes.len() * 8) as u128;
        bytes.push(0x80);
        ...
        for i in bytes.chunks(8) {
            ...
            vec.push(u64::from_be_bytes(buf));
        }
        vec
    }
    ...
    pub fn digest(&self) -> Vec<u8> {
        ...
        if let Sha3Type::Sha512 = self.sha_type {
...
mod tests {
    ...
    fn sha512_test() {
        ...
        assert_eq!(sha.hexdigest(), "374D794A95CDCFD8B35993185FEF9BA368F160D8DAF432D08BA9F1ED1E5ABE6CC69291E0FA2FE0006A52570EF18C19DEF4E617C33CE52EF0A6E5FBE318CB0387".to_string());

        assert_eq!(sha.digest(), vec![0x37, 0x4D, 0x79, 0x4A, 0x95, 0xCD, 0xCF, 0xD8,
        ...
        let mut sha1 =  Sha3::sha512();
        sha1.update("Bananensaft".to_string());
        assert_eq!(sha1.hexdigest(), "69E3637EB21E2AA5D6E5205A21DBA39F38E5FC65388B9BE6E85C2ADEDCF69C2347A374ED7C4D5B4AB48F102B9A1CE7720F44F565B3AA40FC4662CC391BE294F5".to_string());
        sha1.update("Schokokuchen".to_string());

        let mut sha2 =  Sha3::sha512();
    ...
    fn sha384_test() {
        ...
        sha.update("Hello, World!".to_string());
        assert_eq!(sha.hexdigest(), "5485CC9B3365B4305DFB4E8337E0A598A574F8242BF17289E0DD6C20A3CD44A089DE16AB4AB308F63E44B1170EB5F515".to_string());

        assert_eq!(sha.digest(), vec![0x54, 0x85, 0xCC, 0x9B, 0x33 ,0x65, 0xB4, 0x30, 
